...
the Databricks integration GUID by name. Falls back to
...
def server(i: Inputs, o: Outputs, session: Session):
    ...
    def load_all():
        ...
        integration_name = os.environ.get("DATABRICKS_INTEGRATION_NAME")
        if integration_name:
        ...
        if db_assoc is None:
            label = f"name={integration_name}" if integration_name else "type=DATABRICKS"
            ui.notification_show(
                f"No Databricks integration found ({label})", type="error"
            )
            return
