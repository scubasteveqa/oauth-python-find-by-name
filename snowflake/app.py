...
the Snowflake integration GUID by name.
...
def server(i: Inputs, o: Outputs, session: Session):
    ...
    def load_all():
        ...
        integration_name = os.environ["SNOWFLAKE_INTEGRATION_NAME"]
        sf_assoc = current_content.oauth.associations.find_by(
            name=integration_name
        )
        if sf_assoc is None:
            ui.notification_show(
                f"No Snowflake integration found with name: {integration_name}",
                type="error",
            )
            return
