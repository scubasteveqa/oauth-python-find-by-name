...
both Snowflake and Databricks integration GUIDs by name. Falls back
...
app_ui = ui.page_sidebar(
    ui.sidebar(
        ...
        ui.tags.p(
            "Snowflake + Databricks via find_by(name=...)",
    ...
    title="Multi-Source Analytics (find_by name)",
)
...
def server(i: Inputs, o: Outputs, session: Session):
    ...
    def load_all():
        ...
        # Discover integrations: by name if provided, otherwise by type
        sf_name = os.environ.get("SNOWFLAKE_INTEGRATION_NAME")
        db_name = os.environ.get("DATABRICKS_INTEGRATION_NAME")

        if sf_name:
            sf_assoc = current_content.oauth.associations.find_by(name=sf_name)
        else:
        ...
        if db_name:
            db_assoc = current_content.oauth.associations.find_by(name=db_name)
        else:
        ...
        if sf_assoc is None or db_assoc is None:
            sf_label = f"name={sf_name}" if sf_name else "type=SNOWFLAKE"
            db_label = f"name={db_name}" if db_name else "type=DATABRICKS"
            ui.notification_show(
                f"Missing: snowflake({sf_label})={sf_assoc is not None}, "
                f"databricks({db_label})={db_assoc is not None}",
                type="error",
            )
            return
