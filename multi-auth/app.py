...
both Snowflake and Databricks integration GUIDs by name.
...
def server(i: Inputs, o: Outputs, session: Session):
    ...
    def load_all():
        ...
        sf_name = os.environ["SNOWFLAKE_INTEGRATION_NAME"]
        db_name = os.environ["DATABRICKS_INTEGRATION_NAME"]
        ...
        if sf_assoc is None or db_assoc is None:
            ui.notification_show(
                f"Missing integration: snowflake({sf_name})={sf_assoc is not None}, "
                f"databricks({db_name})={db_assoc is not None}",
                type="error",
            )
            return
